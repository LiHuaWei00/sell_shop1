...
import com.imooc.sell_shop.converter.OrderForm2OrderDTOConverter;
...
import com.imooc.sell_shop.util.ResultVOUtil;
...
/**
 * @ Author     ：李华伟
 * @ Date       ：Created in 9:56 2018/7/24
 */
@RestController
...
public class BuyerOrderController {
    ...
    public ResultVO<Map<String,String>> create(@Valid OrderForm orderForm, BindingResult bindingResult){
        if(bindingResult.hasErrors()){
            ...
            throw new SellRxception(ResultEnum.    PAEAM_ERROR.getCode(),bindingResult.getFieldError().getDefaultMessage());
        }
        OrderDTO orderDTO = OrderForm2OrderDTOConverter.convert(orderForm);
        if(CollectionUtils.isEmpty(orderDTO.getOrderDetailList())){
            log.error("[创建订单]购物车不能为空");
            throw new SellRxception(ResultEnum.CART_EMPTY);
        }
